pipeline {
    ...
    stages {
        ...
        stage ('Invoke Ansible deploy_to_ci playbook') {
            steps {
               sh 'ansible-playbook -i /tmp/inventory /tmp/deploy_to_ci.yml -u ubuntu --private-key /home/ubuntu/.ssh/gradwork.pem -e "artifactory_url=${artifactory_url} build_number=${BUILD_NUMBER}"'
            }
        }
    }
}
